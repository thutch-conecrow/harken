...
function CustomComponent() {
  ...
  return (
    ...
    >
      <ThemedText variant="body">Styled with theme tokens</ThemedText>
    </View>
  );
}
```
...
```tsx
<FeedbackSheet containerStyle={{ maxHeight: 500 }} contentStyle={{ paddingTop: 8 }} />
```
...
```tsx
import { HarkenProvider, createTheme, lightTheme, darkTheme } from "@harkenapp/sdk-react-native";
