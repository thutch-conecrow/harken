...
describe('resolveTheme', () => {
  ...
  describe('component token fallbacks', () => {
    ...
    describe('upload tokens', () => {
      ...
      it('uses default progress track color', () => {
        const resolved = resolveTheme(lightTheme);
        expect(resolved.colors.uploadProgressTrack).toBe('rgba(255, 255, 255, 0.3)');
      });
    });
  ...
  describe('sizing defaults', () => {
    it('provides default buttonMinHeight', () => {
      const resolved = resolveTheme(lightTheme);
      expect(resolved.sizing.buttonMinHeight).toBe(48);
    });

    it('provides default inputMinHeight', () => {
      const resolved = resolveTheme(lightTheme);
      expect(resolved.sizing.inputMinHeight).toBe(44);
    });

    it('provides default tileSize', () => {
      const resolved = resolveTheme(lightTheme);
      expect(resolved.sizing.tileSize).toBe(80);
    });
  ...
  describe('opacity defaults', () => {
    it('provides default disabled opacity', () => {
      const resolved = resolveTheme(lightTheme);
      expect(resolved.opacity.disabled).toBe(0.6);
    });

    it('provides default pressed opacity', () => {
      const resolved = resolveTheme(lightTheme);
      expect(resolved.opacity.pressed).toBe(0.8);
    });
...
describe('createTheme', () => {
  describe('sizing and opacity merging', () => {
    ...
    it('returns base theme unchanged when no overrides provided', () => {
      ...
      const result = createTheme(baseWithExtras);

      expect(result).toBe(baseWithExtras);
    });
  });
});
